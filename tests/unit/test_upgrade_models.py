...
def service(tmp_path, monkeypatch):
    """SelfEditService against a throwaway git repo."""
    import subprocess
    ...
    allowlist.write_text(json.dumps({"allow": ["README.md"], "deny": []}))
    monkeypatch.setenv("JARVIS_SELFEDIT_ALLOWLIST", str(allowlist))
    return SelfEditService(repo)
...
def clean_profile_env(monkeypatch):
    for var in ("JARVIS_UPGRADE_PROFILE", "JARVIS_UPGRADE_MODELS", "MOONSHOT_API_KEY", "ANTHROPIC_API_KEY"):
...
def test_legacy_mode_without_registry(service, monkeypatch, tmp_path):
    cfg = tmp_path / "upgrade_agent.yaml"
    cfg.write_text(yaml.safe_dump({"model": "gpt-4.1-mini", "temperature": 0.2, "max_iterations": 1}))
    monkeypatch.setenv("JARVIS_UPGRADE_MODELS", str(tmp_path / "nonexistent.yaml"))
    client = CapturingClient()
    agent = ua.UpgradeAgent(service, config_path=cfg, client_factory=lambda: client)
    assert agent.profile_name is None
    assert agent.model == "gpt-4.1-mini"
    assert agent.model_label() == "gpt-4.1-mini"
    agent.run("change the readme")
...
def test_registry_loads_and_default_resolves(service, registry_file):
    client = CapturingClient()
    agent = ua.UpgradeAgent(service, registry_path=registry_file, client_factory=lambda: client)
...
def test_explicit_profile_beats_env_beats_default(service, registry_file, monkeypatch):
    ...
    agent = ua.UpgradeAgent(service, registry_path=registry_file, client_factory=CapturingClient)
    assert agent.profile_name == "claude-opus"  # env beats default
    agent2 = ua.UpgradeAgent(
        service, registry_path=registry_file, profile="kimi-k2", client_factory=CapturingClient
    )
...
def test_unknown_profile_raises_with_available_names(service, registry_file):
    with pytest.raises(ua.UnknownModelProfileError) as exc:
        ua.UpgradeAgent(service, registry_path=registry_file, profile="gpt-99", client_factory=CapturingClient)
...
def test_temperature_null_is_omitted_from_request(service, registry_file):
    client = CapturingClient()
    agent = ua.UpgradeAgent(service, registry_path=registry_file, profile="kimi-k2", client_factory=lambda: client)
...
def test_temperature_sent_when_set(service, registry_file):
    client = CapturingClient()
    agent = ua.UpgradeAgent(service, registry_path=registry_file, profile="claude-opus", client_factory=lambda: client)
    agent.run("touch the readme")
    assert client.requests[0]["temperature"] == 0.2
...
def test_available_models_reports_key_presence_without_material(service, registry_file, monkeypatch):
