...
UI_CONTROL_SCHEMA = {
    ...
    "function": {
        ...
        "description": (
            ...
            "display window / 'the big screen' / 'the other screen'), "
            "display_close (close it / 'show it here'), overlay_dismiss "
            "(dismiss the on-stage content panel / 'close that'), "
            "mic_mute (stop listening), wake_on, wake_off. There is "
            "deliberately no mic_unmute: while muted the user cannot be "
            "heard, so the command could never arrive — unmuting is the "
            "wake word's or the mic button's job."
        ),
        "parameters": {
            "type": "object",
            "properties": {
