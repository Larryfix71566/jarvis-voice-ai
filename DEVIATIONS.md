...
- Update 2026-09-22 (docs reconciliation): the adaptation is incomplete in one place — `mcp_servers/mcp_selfedit/server.py` still imports `from mcp.server.fastmcp import FastMCP` (unchanged since it was added in `33b78f6`, 2026-08-10); the other twelve `mcp_servers/*/server.py` files use `from fastmcp import FastMCP`. Both resolve against the current lock (`mcp==1.29.0`, `fastmcp==3.4.5`). Recorded, not changed here
...
- Update 2026-09-22 (docs reconciliation): `jarvis/bot/pipeline.py` now passes `should_interrupt=False` to `DeepgramFluxSTTService` (changed 2026-08-22, `88d58bc`). With `True`, Flux broadcast an interruption on every VAD-level StartOfTurn, before any transcript or speaker score existed; interruptions now come only from the user-turn-start strategy (`SpeakerVerifiedMinWordsTurnStartStrategy`, `jarvis/bot/speaker_gate.py`). This is a product change layered on the D-004 wiring, not a library mismatch; the import/`FunctionSchema`/`VADProcessor` adaptations above are unchanged
- Architecture impact: none
...
- Update 2026-08-06 (receive path): Phase 5 acceptance found the transport-level `on_app_message` event never dispatches to bot-registered handlers on this runner stack — the inbound message reaches the pipeline as an `InputTransportMessageFrame` (the runner-added RTVIProcessor logs "Ignoring not RTVI message") but the transport event handler never fires (verified with a first-line debug print and a minimal probe). The voice/set handler is now ALSO registered on the WebRTC connection's own `"app-message"` event (`runner_args.webrtc_connection`, passed into `run_session`) — the same event the transport itself subscribes to. Verified live: probe `{"type":"voice/set","voice":"eric"}` → handler runs → `{"type":"voice/current","voice":"eric"}` echo. Both registrations share one `handle_voice_set` implementation; locked shapes unchanged. The acceptance harness additionally sends 1 Hz `"ping"` keepalives on the data channel — pipecat's `SmallWebRTCConnection.is_connected()` requires a ping within the last 3 s (client-js pings natively), otherwise inbound messages are queued instead of handled
- Update 2026-08-14 (docs/plans/implemented/MORTIMER_AGENT_TRUST_PLAN.md D19): the dead `@transport.event_handler("on_app_message")` registration confirmed-never-firing by the 2026-08-06 update above has been deleted from `jarvis/bot/pipeline.py`, gated on the plan's exact precondition (`grep -n 'event_handler("app-message")' jarvis/bot/pipeline.py` returning exactly one match — the working connection-level handler — before deletion; regression test `test_dead_on_app_message_handler_removed` in `tests/integration/test_bot_wiring.py` re-checks the same count on every run). Only the working connection-level `"app-message"` handler remains; `handle_voice_set`'s implementation and both locked message shapes are unchanged.
- Update 2026-09-22 (docs reconciliation): a second receive path exists for the native macOS client, which talks to the bot over the runner's `/ws-client` WebSocket (`FastAPIWebsocketTransport`, `jarvis/bot/ws_transport.py`, added `71e449f` 2026-09-12). That transport has no connection object and no `on_app_message` event; inbound messages arrive in the pipeline as `InputTransportMessageFrame`, so `bot()` (`jarvis/bot/bot.py`) inserts a `ClientMessageProcessor` directly after `transport.input()` and `run_session` binds it to the same handlers the WebRTC connection-level `"app-message"` event uses. The WebRTC path does not get the processor (both would fire). Outbound messages keep the `rtvi-ai` wrapper; `ProtobufFrameSerializer` carries them. Locked payload shapes unchanged
- Architecture impact: none
...
- Update 2026-09-22 (docs reconciliation): the (c) figures are historical. `requirements-lock.txt` at `88b206f` pins 198 packages (last changed `0b95d4c`, 2026-09-09) and has `websockets==16.1.1`; `pipecat-ai==1.4.0`, `mcp==1.29.0`, `fastmcp==3.4.5`, `fastapi==0.141.1`, `starlette==1.3.1` and `httptools==0.8.0` are as stated above. The 180/26 gate counts belong to Phase 8 and were not re-run for this note
- Architecture impact: none
...
- Adaptation: user-side transcript logging moved to a task-level observer — `TranscriptObserver(BaseObserver)` in `jarvis/bot/transcript_log.py`, attached via `PipelineTask(pipeline, observers=[...])`. Observers see every frame at every hop without touching pipeline topology; the locked 9-processor order is unchanged and TranscriptLogger keeps its locked position and its assistant-side duties (JARVIS lines, TURN llm_done latency lines, assistant persistence). `scripts/spoken_acceptance.py` added as test tooling (plays WAV files over /api/offer using only locked deps) so the Phase 4 spoken checklist can run without a human at the mic
- Update 2026-08-06 (same root cause, Phase 5): the `TURN user_end->first_audio` line was implemented processor-side in TranscriptLogger but could never fire — in the locked order TranscriptLogger sits UPSTREAM of the TTS service, and OutputAudioRawFrame is born at the TTS service and flows only downstream. first_audio timing moved to the same TranscriptObserver (turn start on UserStoppedSpeakingFrame, single-shot line on first OutputAudioRawFrame; regression test `test_observer_logs_first_audio_latency`). TranscriptLogger keeps llm_done, which does flow through it
- Architecture impact: none
...
### D-012 — 2026-08-17
- Plan reference: `docs/plans/implemented/MORTIMER_VOICE_UI_PLAN.md` U4 ("never more than ~40% of the viewport" cap on the in-page display overlay), superseded by `docs/plans/MORTIMER_PLAN_REVIEW_AND_DOCS_PLAN.md` W1
- Specified: `.display-panel` never exceeds `max-width: 40vw; max-height: 40vh` — an ambient-visibility default so the ready-wave stays visibly alive behind any displayed content
- Installed reality: not a library mismatch — a deliberate, Larry-approved product decision. The overlay showing a plan/review document needed to be user-resizable and could legitimately need more than 40% of the viewport (e.g. reading a full plan) once the user has explicitly dragged it larger
- Adaptation: `DisplayPanel.tsx` gained a drag-resize handle (`.display-resize`, SideDrawer.tsx's pointer-capture pattern); once the user resizes, the resulting `{w, h}` is applied as inline `width`/`height` and persisted to `localStorage` under `mortimer.display.size`, clamped to `[280×180, 0.9×innerWidth × 0.85×innerHeight]` — this inline size deliberately OVERRIDES the CSS 40vw/40vh default. A never-resized window is unaffected: no inline size is set, so the 40vw/40vh U4 default still governs it exactly as before
- Update 2026-09-22 (docs reconciliation): this entry now describes only the frozen web console (`web/`, frozen 2026-09-04). The native client's display panels (`macos/MortimerHost/Sources/MortimerHost/Display/DisplayWindowStore.swift`) have no 40%-of-viewport cap (in the default layout 2, unpinned results tile in a bounded supporting stage and pinned overflow uses these panels): panels open at 520×400 pt, `minPanelSize` 280×200 is the only limit ("there is no maximum", 2026-09-05), a larger panel is clipped by the viewport, and double-click fits the panel to the viewport; the size is not persisted to storage
- Architecture impact: none
