...
Status: **implemented** (spec P5, branch `impl/p5-registry-split`, 2026-09-23) except step 7, which is a live action on the Mac — see the implementation record after §6. Originally written 2026-09-13 after a live session in which the developer sub-agent was refused twice while trying to add new models ("The registry file is still locked to human review — it's in the deny list, so self-edits can't touch it", 17:01:45).
...
### Implementation record (P5, 2026-09-23)

Carried out per `SPEC.md` P5 with amendments A1–A5; one commit per step, A2's second bullet, A3 and A5 after the rebase onto P2–P7 (item 9 below).

- **Step 1.** §3 answered above. Before any code moved, the last single-file registry was frozen (`tests/unit/fixtures/model_registry/upgrade_models.pre_split.yaml`) with what the pre-split loader returned for it (`registry.pre_split.json`) and what `available_models()` rendered (`available_models.pre_split.json`), and the byte-identity and D6 tests were written against the real config.
- **Step 2.** `load_model_registry()` joins, and accepts three shapes: the legacy single file; the split pair; a whole split registry in one file (the §9 concatenation). Default resolution is the split pair whenever **either** split file exists, and the legacy `upgrade_models.yaml` only when neither does, so the legacy shape is unreachable by default while the denied endpoints file exists, and a missing endpoints file is a `ModelRegistryError`. Split files beside a legacy file are refused as ambiguous. A file named `model_profiles.yaml` can never load in the legacy shape or carry `endpoints:`/`supervisor:`. Duplicate profile names are a load error in the split shape. A missing registry still returns the empty registry.
- **Step 3.** Converted: `jarvis/model_catalog.py`, `jarvis/skills/registry.py` (dynamic env source `upgrade_models_api_keys`, name kept), `jarvis/vault.py` `verify`, `scripts/check_env.py` (three sites), `scripts/check_keys.py`, and `jarvis/model_routing.py::_load_model_registry` (A2). `scripts/voice_model_bench.py` never parsed the registry (docstring only). `scripts/run_memory_provider_shadow.py` defaulted `--registry` to the single file and now defaults to the loader. `tests/unit/test_model_registry.py` and `test_model_floor.py` parsed the YAML themselves and now read the joined view. A static test fails if any module other than the loader and the split script names a registry file path. `check_env` output diffed before/after: identical.
- **Step 4.** `scripts/split_model_registry.py` renders the pair (profiles keep every comment: only each profile's `provider`/`base_url`/`api_key_env` lines become one `endpoint:` line) and refuses to write unless the join equals the source. Endpoints: `moonshot`, `anthropic`, `codex-subscription` (`kind: subscription`, `route: codex_subscription`, A1), `openrouter`; `supervisor:` pins `anthropic/claude-fable-5` on `anthropic`. `config/upgrade_models.yaml` deleted.
- **Step 5.** D3 and D4 tests written failing-first (13 failed), then the checks were added to the loader.
- **Step 6.** Allowlist per D5. The UpgradeAgent system prompt and `jarvis/selfedit/allowlist.py` called "the model registry" human-only; both now name the endpoints file, so the planner will not decline step 7's goal.
- **Step 7. Not done here.** It needs the live loop on Larry's Mac (a sandbox self-edit PR from the developer adding one profile on an existing endpoint). Suggested goal: *"Add an OpenRouter profile for <model> to config/model_profiles.yaml on the openrouter endpoint, tier <tier>, with identity <vendor/model>."*. The PR will change only `config/model_profiles.yaml`. From then on the two migration-pinned tests skip, saying why. The step-7 PR will also show a check_env warning that the new identity is not yet in the catalogue; that is expected until the sync runs.
- **Step 8.** The split script seeds `config/generated/model_catalog.<endpoint>.json` (four files, 14 entries, every §4a field present, `fetched_at: null`, `source: "seeded"`), and `check_env` reports each endpoint's catalogue age (WARN while seeded or older than 3 days). It also warns per profile when the profile is missing from its endpoint's catalogue, disagrees on the model string, or is marked for retirement. A profile on a model retired on or before today fails CI (§4a.3).

**Deviations, each with its reason:**

1. **The loader lives in `jarvis/agents/upgrade_agent.py`, and its callers import it lazily.** The loader enforces D3, so it has to stay Tier 0. A new core-tier module would have let the loop weaken the boundary. `jarvis/model_routing.py` is imported by `upgrade_agent.py`, so it imports the loader inside the function to avoid a cycle.
2. **Every converted reader now honours `JARVIS_UPGRADE_MODELS`.** Before, `skills/registry.py`, `vault.py`, `check_env.py` and `check_keys.py` always read `config/upgrade_models.yaml`. Now they share the loader's precedence. The variable is unset in production.
3. **D3 covers more keys than `base_url`/`api_key_env`.** It also covers `provider` (joined from the endpoint, D2), `routes` (`model_routing._route_for_profile` reads a per-profile route's own `base_url`/`credential_env`), `api_key`, `credential_env`, `api_base`, `host`, `url` and the header keys, at any depth, plus any value containing `://`. The endpoints file is strict too: unknown keys are an error.
4. **D4 is enforced at load time as well as by a test.** The load check covers identity and endpoint. The pin carries no model string, so the model-string half of D4 is checked by comparing against the generated catalogue (`test_the_pinned_planner_model_string_matches_the_catalogue`).
5. **Profiles keep `model` and `timeout_s`.** D1's list omits them. `model` stays authoritative on the profile because the D6 test requires the join to return exactly the pre-split keys. Joining catalogue facts into the returned profiles would change that shape, so `load_upstream_catalogs()` is a separate read-only reader.
6. **There are four catalogues and 14 entries, not three and 13** (A1: `codex-subscription` is its own endpoint).
7. **The split script is kept, not deleted (§5).** The permanent D6 proof re-runs it over the frozen pre-split file, and §9 keeps legacy acceptance for a release. Delete both together.
8. **The two real-config D6/byte-identity tests are pinned to the migration by the split files' sha256.** Once the pool legitimately changes they skip with a reason, instead of blocking step 7. What they prove stays covered by the script-over-frozen-file test and the concatenated-rollback test, both of which always run.
9. **A2 (second bullet), A3 and A5 landed after rebasing onto P2–P7.** A2: `test_status_modules_parse_no_registry_file_themselves` (static) and `test_every_status_registry_reader_calls_load_model_registry` (a spied loader). A3: `jarvis/status/daily.py` renders `model_catalog.<endpoint>.json` into `data/status/generated/` after the catalogs; it maps an endpoint to the discovered provider serving its profiles and renders only when that provider's list came from the endpoint's own base URL; a skipped endpoint keeps its previous file (§4a.2). `identity` is the configured profile's when a profile uses the model string, else the id when vendor-qualified, else `<vendor>/<id>` with the vendor its profiles already use. Prices: OpenRouter's per-token strings become `*_price_per_mtok`; negative means variable and is null. The fetchers carry them in a new `CatalogResult.facts` (id -> pricing/context_length), not in the model entries, because `test_openai_compatible_parse` pins those to three keys; `as_payload()` keeps `facts` out of the sidecar/MCP payloads and the daily snapshot. The daily job also reads `load_registry_layers()` (the loader's unjoined half) for endpoint ids. A5: `test_real_config_is_the_split_registry_and_has_no_coverage_gaps`.
