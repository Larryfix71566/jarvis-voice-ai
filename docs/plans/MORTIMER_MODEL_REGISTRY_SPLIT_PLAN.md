...
2. **Fixtures — legacy single-file acceptance, not a fixture rewrite.** 25 test files point `JARVIS_UPGRADE_MODELS` (16 `setenv` sites) or a `registry_path` argument at a temporary single-file registry whose profiles carry `base_url`/`api_key_env`. The loader keeps accepting that shape when the file *is* the whole registry: no `endpoints:` section, no profile naming an `endpoint:`, and not a file named `model_profiles.yaml`. The default resolution never reaches the legacy shape while `config/model_endpoints.yaml` exists (see §6 step 2 notes), so a routine edit cannot smuggle a credential-bearing registry back in by recreating `config/upgrade_models.yaml`.
