...
Carried out per `SPEC.md` P5 with amendments A1, A2 (first bullet) and A4; one commit per step.
...
9. **Not done on this branch:** A3 (the daily job also rendering catalogues into `data/status/generated/`) depends on `jarvis/status/daily.py` from P4. A2's second bullet (the `jarvis/status/*` readers) and A5 (`test_real_config_has_no_coverage_gaps`) depend on P2. `jarvis/status/` does not exist on this base. Both are for after those phases merge.
