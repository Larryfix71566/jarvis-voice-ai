...
**Status:** **IMPLEMENTED (code) 2026-09-04 — §8 VERIFICATION IN PROGRESS, 2026-09-05.** Every GC item is in the tree: GC1b (`VALIDATE_PYTEST_TIMEOUT_S` 900, `selfedit/service.py:51`), GC2 (`REPO_MAP.md` names all nine previously-missing modules), GC3 (ROADMAP's Electron line gone), GC4 (`mortimer.sh` no longer launches web; `test_web_freeze_preflight.py` refuses a web-only goal), GC5 (`bot/pipeline.py:1130`), GC6 (judge error string, `temperature: null`, `agreement.py:188 supersede_score_rows`), GC7 (`scripts/backup_db.py`, `scripts/launchd_gen.py`), GC8 (`jarvis/tenant.py`, `tests/unit/test_tenant_columns.py`), GC9 (migration 0021, `admin/reminder_notifier.py`, `jarvis/notify.py`), GC10 (root `.wav` files gone). §8, run on Larry's hardware 2026-09-05: **8.2/8.3 session checks PASS** (two analyst delegations, no "analyst" leak, zero `**` in TTS, barge-in `late_result_neutralized count=1`); **8.5 MEASURED** — voice $0.2239 vs LLM $0.3647, **ratio 0.61, so snapshot item 9 did NOT hold** (but note the 09-03 review measured 3-4x on a conversational workload; the ratio swings with what the session does, and it is n=1 either way — do not buy hardware on it); **8.7 COMPLETE** — the `d6e0059b` replay ran, all five frontier judges answered, `superseded shadow rows: 20`, frontier abstention 0.4 → 0.0, GC6(b) verified; **8.8 COMPLETE** — 0020/0021 applied 15:41:01Z, `memories.user_id` present, 14 runs logged after; **8.9 COMPLETE** — notified at +76 s while disconnected, then spoken exactly once on reconnect. STILL OPEN: §8.3 speaker gate (offline), §8.4 supervision install (backups themselves now exist and verify clean — `integrity_check ok`, 36/36 rounds, 414/414 runs), §8.6 self-edit half (suite times at 52.8 s against the 900 s gate, but no self-edit has reached validation), §8.10 hygiene, and the GC6(d) kimi-k3 decision. THREE DEFECTS FOUND BY RUNNING THE VERIFICATION, none of them in this plan: `jarvis/council/__main__.py` never loaded vault credentials, so `--replay` — GC6(b)'s own repair tool — had been dead since it was written (fixed, `0084d23`); every self-edit still runs `npm ci` + `npm run build` on the FROZEN web client with no gate (`selfedit/service.py:550-555`), which can fail validation and convene an E1 council over a component nobody runs; and a barge-in during a delegation can duplicate the next write — one store request produced two `delegate_task` calls and two notes on 2026-09-05. Original header: DRAFT for Larry's approval, 2026-09-04. Closes every item in `docs/reviews/ARCHITECTURE_SNAPSHOT_2026-09-04.md` §0 — by code in this plan, by a Larry-only step with exact commands, or by naming the existing plan that owns it. Nothing is left as "later" without a trigger.
