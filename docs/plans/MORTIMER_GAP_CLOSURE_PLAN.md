...
**Status:** **IMPLEMENTED (code) 2026-09-04 — §8 VERIFICATION MOSTLY COMPLETE, 2026-09-05.** All GC items are in the tree: GC1b (`VALIDATE_PYTEST_TIMEOUT_S` 900), GC2 (REPO_MAP), GC3 (ROADMAP), GC4 (web freeze), GC5 (`bot/pipeline.py:1130`), GC6 (judge error string, `temperature: null`, `agreement.py:188`), GC7 (`backup_db.py`, `launchd_gen.py`), GC8 (`jarvis/tenant.py`), GC9 (migration 0021 + `reminder_notifier.py` + `notify.py`), GC10 (root `.wav` files gone).

**§8 RESULTS (Larry's hardware, 2026-09-05).** §8.2 session checks PASS — two analyst delegations, no "analyst" leak, zero `**` in TTS, barge-in `late_result_neutralized count=1`. §8.4 **COMPLETE** — first backups ever taken and verified (`integrity_check ok`, 36/36 rounds, 414/414 runs); launchd live with five pids; both supervision proofs pass (`mortimer.sh` exits 3; killing the bot respawns it in <10 s); vault key exported off-machine. §8.5 **MEASURED** — voice $0.2239 vs LLM $0.3647, **ratio 0.61, so snapshot item 9 did NOT hold** — but the 09-03 review measured 3–4× on a conversational workload, so the ratio swings with the session and is n=1 both ways; do not buy hardware on it. §8.7 **COMPLETE** — `d6e0059b` replayed, all five frontier judges answered, `superseded shadow rows: 20`, frontier abstention 0.4 → 0.0, GC6(b) verified. §8.8 **COMPLETE** — 0020/0021 applied 15:41:01Z, `memories.user_id` present, 14 runs logged after. §8.9 **COMPLETE** — notified at +76 s while disconnected, spoken exactly once on reconnect, `delivered` 0 → 1.

**PRECONDITION §8.4 DID NOT STATE, and it cost a broken install: the repo MUST NOT live under `~/Documents`.** That folder is TCC-protected. Terminal.app holds a grant, which is why `./scripts/mortimer.sh` always worked, but a launchd-spawned process inherits none — every job died with `getcwd: ... Operation not permitted` and the stack went down with six jobs showing `loaded (idle)`. The 03:15 backup would have failed the same way, silently, forever. Repo moved to `~/jarvis-voice-ai-clean` (sibling of `~/mortimer-vault`, which never had the problem) rather than granting Full Disk Access to `/bin/bash` and `/usr/bin/python3`. The backup job now runs `/usr/bin/python3`, not uv's relocatable managed interpreter, with `test_backup_db_uses_only_stdlib` enforcing what makes that safe.

**GC6(d) kimi-k3 — the evidence now says DEMOTE, not the plan's default of keep.** Across six rounds it produced 12 null scores in 24 rows (50%), alternating rather than trending: failed 08-23 ×2, worked 08-30 ×2, failed 09-01, worked 09-05. Every other mid judge is at zero — or-gpt-5.1 0/24, or-grok-4.3 0/24, or-sonnet-5 0/20, claude-sonnet-5 0/4 — so **100% of mid-tier abstentions in the entire history are kimi-k3**, and the 0.125 rate is one profile, not noise. A judge that returns nothing half the time silently shrinks a 4-judge panel to 3, and because the rows exist with null scores the short-handed metric reads 0. Caveat: demotion moves it to the proposer pool rather than fixing it; a 50% timeout rate against one provider suggests the timeout is too tight for Moonshot, and GC6(a) means the next failure will finally carry a real error string.

**STILL OPEN:** §8.3 speaker gate (offline); §8.6 self-edit half (suite times at 52.8 s against the 900 s gate, but no self-edit has reached validation — staging TTL is 600 s, so re-stage and confirm promptly); §8.10 hygiene; the GC6(d) edit itself (`config/upgrade_models.yaml` is human-only).

**THREE DEFECTS FOUND BY RUNNING THIS PLAN, none of them in it.** (1) `jarvis/council/__main__.py` never loaded vault credentials, so `--replay` — GC6(b)'s own repair tool — had been dead since it was written; fixed `0084d23`. (2) Every self-edit still runs `npm ci` + `npm run build` on the FROZEN web client with no gate (`selfedit/service.py:550-555`), unlike the `core_changed` gate three lines above; it can fail validation and convene an E1 council over a component nobody runs. STILL OPEN. (3) A barge-in during a delegation duplicated the next write — one store request produced two `delegate_task` calls and two notes; fixed `2a13857`. Also: `requirements.txt` line 1 had NO version pin, so the venv rebuild during the move resolved pipecat 1.8.1 and dropped the ElevenLabs SDK; pinned in `54abcd6`. Original header: DRAFT for Larry's approval, 2026-09-04.
