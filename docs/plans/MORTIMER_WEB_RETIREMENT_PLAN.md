...
- **C10** (degradation-proof) — honoured: every removal is specified by text-anchored edit, every "X references web/" claim is grep-verified in §1, and §0 is a hard stop with five mechanical checks (four STOP-and-report, one non-blocking state flag — review F2/F4).
...
(`review-opus/MORTIMER_WEB_RETIREMENT_PLAN.review.md`, 2 BLOCKER / 2 MAJOR /
3 MINOR). This plan had never been reviewed before (it was written after the
first review round); this is the first pass, not a re-check. Every citation
and grep command below was re-run against the repo snapshot, not re-read.

| Finding | Sev | Section(s) changed | What changed |
|---|---|---|---|
| F1 | BLOCKER | §1 R6, §3 W6, §5 Step 5 | `.github/workflows/validate.yml` and `scripts/validate.yml` are NOT identical mirrors — re-measured: the live workflow's frontend comment is `# Gate 5:`, the staging copy's is `# Gate 4:` (it's missing Gate 3b and the latency gate entirely, per its own "one-time manual copy" header). Step 5 now gives each file its own anchor text instead of one shared string, and STOPs per-file (B0.3) if either doesn't match, rather than silently mismatching on the staging copy. |
| F2 | BLOCKER | §0 B0.1 (new gate 4), B0.3, §5 Step 2, §7 T4, §10 RM-7/RM-8 (new) | This plan assumed REMOTE (T2) had already landed, but the roadmap runs T1.3/T1.4 and T2 in the same wave with no ordering guarantee, and the current snapshot confirms REMOTE genuinely hasn't landed here (no `BearerAuthMiddleware`, no `jarvis_token` in localStorage). Added a non-blocking `REMOTE_LANDED` state check (B0.1.4) threaded through Step 2's test (skipped, not failed, if REMOTE absent) and RM-7's "risk that closes" claim (now conditional). CORS removal itself proceeds unconditionally — it's safe either way (RM-2). |
| F3 | MAJOR | §5 Step 6, §5 Step 7 (new second pass) | The `README.md:270` `mortimer.sh` tree-listing comment (`bot+admin+web`) and the `:345` troubleshooting-row phrasing were unlisted in Step 6's edit instructions and unreachable by Step 7's grep patterns (a bare `(bot+admin+web)` parenthetical matches none of them) — re-measured: confirmed both lines exist and confirmed the sweep patterns don't catch either. Added both to Step 6's explicit edit list, and added a second `\bweb\b` word-boundary sweep over `README.md`/`CLAUDE.md` to Step 7, reviewed by eye. |
| F4 | MAJOR | §0 B0.1 (new gate 5), §3 W7, §10 RM-9 (new), §12 | MAIL's plan expects to land *before* T1.4 (its own text: "Until T1.4 lands, the two tests gate this merge"), but T1.4 has no assigned wave (fires on G1(e) alone) while MAIL is W2 — a real, not hypothetical, ordering hazard, and unlike REMOTE this one is NOT safe to proceed through (deleting `web/` destroys MAIL's edit target with no fallback and no native parity-guard replacement exists). Added a STOP gate (B0.1.5) checking `docs/plans/implemented/MORTIMER_MAIL_CALENDAR_BRIEF_PLAN.md` — the one B0.1 gate Larry may explicitly override, recorded in the PR description. |
| F5 | MINOR | §3 W7 | Dropped the fabricated quotation "F12 offers 'deleted or repurposed'" — re-verified `CROSS_PLAN_RESOLUTION.md`'s actual F12 text contains no such phrase. The underlying delete-not-repurpose decision is unchanged; it now stands on its own stated reasoning without a false citation. |
| F6 | MINOR | §1 R2, §5 Step 3 | `pkill -f "vite"` citation corrected from `:39` to `:34` (re-measured: `grep -n 'pkill -f "vite"' scripts/mortimer.sh` → line 34). "The two occurrences of `bot + admin + web`" corrected to "the occurrence" — re-measured: `scripts/mortimer.sh` has exactly one (line 5); the unspaced form is in `README.md:270`, a different file (also see F3). |
| F7 | MINOR | §5 Step 4 Change B | The replacement comment's own stated purpose ("gate numbering reads honestly") was undermined by leaving the next comment as `# 4.` with no `# 3.` — a skipped number, not honest numbering. Change B now also renumbers that trailing comment to `# 3.`, closing the gap. |

Section/gate renumbering note: §0 B0.1 grew from three STOP gates to five checks (four
STOP, one non-blocking); every in-plan cross-reference (§10 RM-1, the precondition
attestation in §8, §11 items 6/8, §12) was updated to match.
...
**B0.1 — Five mechanical checks: gates 1–3 and 5 STOP-and-report if they fail; gate 4 is a non-blocking state check (review F2/F4). Run all five FIRST.**
...
4. **Whether REMOTE (T2) has landed — check, do not STOP (review F2).** T1.3/T1.4 (this plan's wave) and T2 land in the roadmap's **same wave, explicitly in parallel** (roadmap §5 W1: "T1.1 spike → T1.2 `JarvisKit` → T1.3 macOS app; T2 auth + tunnel … runs beside T1"), so gates 1–3 above passing does NOT imply REMOTE has shipped. Run `grep -q "class BearerAuthMiddleware" jarvis/authmw.py 2>/dev/null`. Record the result as **`REMOTE_LANDED = true|false`** and carry it into Step 2 and §7 T4 below — this is a mode flag, not a stop condition, because removing `CORSMiddleware` is safe either way (no browser client survives this plan regardless of REMOTE's state, RM-2). If `REMOTE_LANDED = false`: Step 2 still runs (CORS removal), but its T4 sub-check is **skipped, not failed** (the test file it names may not exist yet), and RM-7's "risk that closes" claim is conditional — see the corrected Step 2 and RM-7 below.

5. **Whether MAIL (T5) has landed — STOP, unlike gate 4 (review F4).** `test -f docs/plans/implemented/MORTIMER_MAIL_CALENDAR_BRIEF_PLAN.md`. MAIL's own plan states its `web/src/agentLayout.ts` edit and the five→six agent parity test are transitional and expects to land *before* this plan deletes `web/` (*"Until T1.4 lands, the two tests gate this merge"*), but the roadmap assigns T1.4 no wave of its own (it fires whenever G1(e) passes, possibly as early as W1) while MAIL is wave **W2** — so, unlike REMOTE/gate 4, there is a real, non-hypothetical way for this plan to run first. Unlike CORS removal, this is **not safe either way**: if MAIL has not landed, deleting `web/` destroys the file MAIL's plan still needs to edit, with no fallback specified by either plan, and no native replacement for the sixth-agent parity guard is commissioned anywhere (`MORTIMER_NATIVE_CLIENT_APP_PLAN.md` has zero mention of `agents.yaml`/parity checking). If absent → STOP: *"MORTIMER_MAIL_CALENDAR_BRIEF_PLAN.md (T5) is not in docs/plans/implemented/; deleting web/ now would remove agentLayout.ts before MAIL's own plan edits it, and no plan re-establishes the sixth-agent parity guard natively. Sequence T5 first, or get Larry's explicit sign-off to proceed anyway and accept the guard is gone until a future plan re-adds a native equivalent (none is named today)."* This is the one B0.1 gate Larry may explicitly override (record the override in the branch's PR description per B0.4) — the other four are not overridable.
...
**B0.3 — Text-anchored edits only, never line numbers.** `MORTIMER_REMOTE_ACCESS_PLAN.md` (W1) *may or may not* have already edited `jarvis/admin/server.py` by the time this plan runs — the two plans share a wave with no ordering guarantee (review F2, B0.1.4). If it has (`REMOTE_LANDED = true`): `BearerAuthMiddleware` is present, `Authorization` was added to `allow_headers`, and middleware was reordered so CORS is outermost. If it has not (`REMOTE_LANDED = false`): today's tree, quoted verbatim in Step 2, is what's actually there. Either way, every edit site in §5 is located by the exact quoted code string given there. If a quoted string is not found verbatim, STOP and report — do not guess a replacement.
...
| R2 | `mortimer.sh` launches web | `scripts/mortimer.sh:26` `WEB_PORT=5173`; `:34` `pkill -f "vite"` (review F6, was mis-cited `:39`); the `nohup ./scripts/run_web.sh …` line; `rotate_log web`; `:103` `check web …`; `:106` `echo "Open http://localhost:$WEB_PORT…"` | One-command stack starter runs bot+admin+**web**. | **W4** — stop starting/stopping/logging/echoing web. |
...
| R6 | CI frontend build | `.github/workflows/validate.yml:41`–`:49` (`# Gate 5: frontend still compiles.` + `setup-node` + `cd web; npm ci; npm run build`) **and** `scripts/validate.yml:44`–`:52` (`# Gate 4: frontend still compiles.` — review F1: NOT an identical mirror; `scripts/validate.yml` is a one-time-manual-copy staging file per its own header comment, and has only four gates, missing Gate 3b and the latency gate entirely) | CI compiles the frontend — Gate 5 in the live workflow, Gate 4 in the staging copy. | **W6** — remove the frontend section from both files, using each file's own gate number as the anchor (they differ). |
...
- **W6 — Remove the frontend CI gate from both `.github/workflows/validate.yml` (Gate 5) and `scripts/validate.yml` (Gate 4 — review F1, the two files' gate numbering differs).** *Why:* the gate compiles a deleted app; left in, CI fails on `cd web`. Removing it drops the `setup-node` step too (nothing else uses Node).

- **W7 — Delete the frontend-parity test rather than repurpose it.** *Why:* the underlying decision stands on its own reasoning, not on a source quote (review F5 — `CROSS_PLAN_RESOLUTION.md`'s F12 never offers a "deleted or repurposed" choice; that phrase does not appear there): the test's whole mechanism is a regex over `web/src/agentLayout.ts`, which no longer exists. A native equivalent (assert every `config/agents.yaml` agent appears in the SwiftUI layout) belongs to the T1.3 plan against its own Swift source, not here — inventing one here would test code this plan does not own. So: delete. **Conditional on ordering (review F4 — see the new §0 B0.1.5 gate below):** MAIL's obligation to edit `agentLayout.ts` + this test when the sixth agent lands is retired by this step ONLY if MAIL has already landed; if it has not, deleting `web/` here removes the file MAIL's own plan still expects to edit, with no fallback specified by either plan.
...
> Run **§0 B0.1's five checks first** (gates 1/2/3/5 STOP-and-report on failure; gate 4 is a non-blocking state flag carried into Step 2/§7 T4/§10 RM-7). If gate 1, 2, 3, or 5 fails, STOP. Do nothing below.
...
**Test that proves it:** `python -c "import jarvis.admin.server"` succeeds (no `NameError` for `CORSMiddleware`). `grep -rn "CORSMiddleware\|5173" jarvis/` returns nothing. **If `REMOTE_LANDED = true`** (B0.1.4): the REMOTE auth tests (`tests/unit/test_authmw.py`) still pass — an unauthenticated `/api/*` call is still 401 (auth is REMOTE's `BearerAuthMiddleware`, unaffected by CORS removal). **If `REMOTE_LANDED = false`** (review F2): skip this sub-check — `BearerAuthMiddleware`/its test file do not exist yet, so there is nothing to assert; today's `/api/*` calls are unauthenticated by design until REMOTE ships, and CORS removal does not change that either way.
...
7. Replace the two trailing echo lines
   ...
   ```
   Update the usage comment at the top of the file: change the occurrence of `bot + admin + web` (line 5 — review F6, there is exactly one, not two; the unspaced `bot+admin+web` form appears only in `README.md:270`, a different file, handled in Step 6) to `bot + admin`.
...
```
Replace the whole block with a single comment (review F7 — this alone does not fix the numbering; Change B below does):
```python
...
```
**Change B — remove the now-unused constant, AND renumber the trailing comment so the sequence has no gap (review F7).** Delete the line `BUILD_TIMEOUT_S = 600` (grep first: `grep -n BUILD_TIMEOUT_S jarvis/selfedit/service.py` must show only the definition once Change A lands; if any other use remains, keep the constant and report). Also change the comment immediately after Change A's block — currently `# 4. Backend unit tests (C2, MORTIMER_MODEL_DISCIPLINE_AND_MAC_SHELL_PLAN.md).` — to `# 3. Backend unit tests (C2, MORTIMER_MODEL_DISCIPLINE_AND_MAC_SHELL_PLAN.md).` (anchor: `# 4. Backend unit tests`). Without this, the comment sequence reads `# 1.`, `# 2.`, (unnumbered), `# 4.` — a skipped "3" — which is exactly the dishonest-numbering outcome Change A's own stated purpose was to avoid.
...
### Step 5 — Remove the frontend CI gate from both files (W6 / R6)
**Files:** `.github/workflows/validate.yml` AND `scripts/validate.yml` — **these are NOT identical** (review F1): `scripts/validate.yml` is a one-time-manual-copy staging file (its own header comment: *"GitHub does not allow API clients without the `workflow` OAuth scope to create files under `.github/workflows/`, so this file ships here instead… ONE-TIME MANUAL STEP: move it into place"*) and has a shorter, differently-numbered gate list (no Gate 3b, no latency gate). **Use a separate anchor per file:**
- **`.github/workflows/validate.yml`:** delete the frontend section anchored on the exact comment `# Gate 5: frontend still compiles.`, the `- uses: actions/setup-node@v4` step with its `with: node-version: "20"`, and the `- name: Frontend build` step with its `run: | / cd web / npm ci / npm run build` body. Nothing else references Node, so `setup-node` goes too.
- **`scripts/validate.yml`:** delete the same shape of section, anchored instead on its own exact comment `# Gate 4: frontend still compiles.` (its Gate numbering is one lower than the live workflow's — do not assume Gate 5 here).
- **If either anchor is not found verbatim:** STOP and report (B0.3) — do not guess a replacement or assume the two files still match each other's gate numbers.
...
**File `README.md`:** remove the web setup step ("Start the web console" / `run_web.sh`), the `cd web && npm run build` reference, the `web/` line in the repo tree, the `run_web.sh` tree line, and the `:5173` rows in the troubleshooting table; in the arch diagram/prose replace "Vite React-TS console" phrasing with the native app; **the `mortimer.sh` tree-listing comment at `README.md:270`, `├── mortimer.sh  # one-command stack starter (bot+admin+web)` → `(bot+admin)`** (review F3 — this line survives Step 7's own sweep unless removed explicitly here, since none of its grep patterns match a bare `(bot+admin+web)` parenthetical); the troubleshooting-table row at `README.md:345` ("Edit mode says the sidecar is offline") also says "starts bot + admin + web together" → "starts bot + admin together"; keep everything about bot/admin/wakeword.
...
**Second pass, bare word boundary (review F3) — catches a stray "web" the patterns above miss (e.g. a `(bot+admin+web)` parenthetical, which matches none of them):**
```
grep -rn "\bweb\b" README.md CLAUDE.md
```
This one is **reviewed by eye, not auto-failed on** — a bare "web" token is too broad to gate mechanically elsewhere in the repo (it can mean "the web" in an unrelated sentence), but in these two specific files, at this point in the plan, every remaining hit should already be accounted for by Step 6's edit list above. Any hit not already covered by a named Step 6 edit is a miss — fix it here before Step 8.
...
| T4 | REMOTE auth suite (`tests/unit/test_authmw.py`), **conditional on `REMOTE_LANDED` (B0.1.4, review F2)** | after Step 2 | if `REMOTE_LANDED = true`: passes — 401 on missing/invalid token unchanged; CORS removal does not touch `BearerAuthMiddleware`. If `REMOTE_LANDED = false`: skipped — the file does not exist yet; not a failure. |
...
1. **Precondition attestation (G1(e)).** Confirm the native app was daily driver 5 consecutive days with zero web-console fallback; that `docs/plans/implemented/MORTIMER_NATIVE_CLIENT_APP_PLAN.md` exists; that `macos/JarvisKit` and `macos/MortimerHost` exist; that `docs/plans/implemented/MORTIMER_MAIL_CALENDAR_BRIEF_PLAN.md` exists or Larry has explicitly signed off on proceeding without it (§0 B0.1.5, review F4). (Gates 1/2/3/5 are §0 B0.1's STOP conditions; gate 4, REMOTE's landed state, is checked but non-blocking (review F2); Larry is the human witness for the five days.)
...
|---|---|---|---|---|
| RM-1 | T1.4 runs before the native app is truly the daily driver; Larry is left with no UI. | low | high | §0 hard stop: three mechanical gates (JarvisKit + MortimerHost present, T1.3 plan in `implemented/`) plus Larry's G1(e) attestation. Delete nothing if any of gates 1/2/3/5 fails (gate 4, REMOTE's state, is non-blocking — RM-8/review F2). |
...
| RM-7 | The retired browser token (`localStorage['jarvis_token']`, XSS-readable — REMOTE A12 / F12 "R9") is thought to persist. | n/a | n/a (positive) | **Conditional on `REMOTE_LANDED` (review F2).** If REMOTE has shipped by the time this plan runs, this *is* a risk that closes: deleting `web/` removes the only writer/reader of that key; no browser storage of a bearer token remains anywhere in the repo (grep for `localStorage` returns nothing after Step 1). If REMOTE has not shipped yet (verified on today's snapshot: `grep -rn localStorage web/src \| grep -i token` → no output), R9 was never live in this tree in the first place — closing it here is vacuous, not a real risk reduction, and REMOTE's own plan retains full ownership of not introducing it once it does land. |
| RM-8 | T2/T1.4 share wave W1 with no ordering guarantee; this plan's §0/§5 previously assumed REMOTE had already landed. | med | low | review F2. §0 B0.1.4 checks `REMOTE_LANDED` mechanically and threads it through Step 2's test and RM-7's claim; CORS removal itself is safe regardless of the flag (RM-2), so this is a documentation-accuracy fix, not a functional one. |
| RM-9 | T1.4 has no assigned wave (fires on G1(e)) while MAIL/T5 is W2; T1.4 could delete `web/src/agentLayout.ts` before MAIL's plan edits it, permanently losing the sixth-agent parity guard with no native replacement commissioned. | med | med | review F4. §0 B0.1.5 STOPs unless `docs/plans/implemented/MORTIMER_MAIL_CALENDAR_BRIEF_PLAN.md` exists — the one overridable gate in B0.1, requiring Larry's explicit sign-off to bypass. |
...
6. **Initialization timing.** N/A — nothing initializes. **Re-walked post-revision (review F2/F4):** two cross-plan orderings matter, not one, and this plan no longer assumes either happened silently — REMOTE (T2) may or may not have landed by the time this plan runs (B0.1.4, checked and threaded through as a mode flag, non-blocking since CORS removal is safe either way) and MAIL (T5) may or may not have landed (B0.1.5, STOP unless present or Larry overrides — proceeding without it is NOT safe, unlike REMOTE). B0.3's text-anchor discipline still handles *within-file* drift for whichever state REMOTE is actually in. ✓ (previously: this item asserted a false premise — see F2/F4 — now corrected)
...
8. **Judgment left to the implementer.** The judgment points are gated deterministically: (a) the precondition — five `test`/`grep` checks, four STOP-and-report (gates 1/2/3/5) and one non-blocking mode flag threaded through downstream steps (gate 4, review F2) — (§0); (b) the grep-sweep residuals — an explicit allow-list of two expected hits, a second bare-word-boundary pass reviewed by eye (review F3), and "any other hit STOP/investigate with the matching decision" (Step 7). No "use judgment" remains. ✓
...
- [ ] `macos/JarvisKit` and `macos/MortimerHost` exist; `docs/plans/implemented/MORTIMER_NATIVE_CLIENT_APP_PLAN.md` exists (§0 B0.1 gates 1-3).
- [ ] `docs/plans/implemented/MORTIMER_MAIL_CALENDAR_BRIEF_PLAN.md` exists, OR Larry has explicitly signed off on proceeding without it and accepts the sixth-agent parity guard is gone until a future plan re-adds a native equivalent (§0 B0.1.5, review F4).
- [ ] `REMOTE_LANDED` recorded (§0 B0.1.4, review F2) and Step 2's T4 / RM-7's claim handled accordingly for whichever value it is.
